        ...
        <div class="d-flex align-items-start industries-nav">
            <div class="nav flex-column nav-pills industries" id="v-pills-tab" role="tablist"
                ...
                @foreach ($industries as $industry)
                    <button @class(['nav-link', 'text-left' , 'active'=> $industry == $industryActive])
                            id="{{ 'v-pills'.$industryActive.'-tab' }}"
                            data-bs-toggle="pill"
                            data-bs-target="#v-pills-home" type="button" role="tab" aria-controls="v-pills-home"
                            aria-selected="true"
                            wire:click="filterIndustry('{{ $industry }}')">
                        {{ $industry }}
                    </button>
                @endforeach
            </div>
            <div class="tab-content h-100" id="v-pills-tabContent">
                <div class="tab-pane fade show active h-100" id="v-pills-home" role="tabpanel"
                     aria-labelledby="{{ 'v-pills'.$industryActive.'-tab' }}">
                    <div class="job_home h-100">
                        <div class="container p-3 h-100">
                            <div class="d-flex justify-content-between me-2 mx-2">
                                <p class="font-weight-bold">{{ $industryActive }}</p>
                                <p class="font-weight-bold">
                                    <a href="{{ route('posts.industry', $industryActive) }}">Xem tất cả
